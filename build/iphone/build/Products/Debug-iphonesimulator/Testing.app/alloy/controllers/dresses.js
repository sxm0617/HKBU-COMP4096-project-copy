function __processArg(obj, key) {
    var arg = null;
    if (obj) {
        arg = obj[key] || null;
        delete obj[key];
    }
    return arg;
}

function Controller() {
    function __alloyId42(e) {
        if (e && e.fromAdapter) return;
        __alloyId42.opts || {};
        var models = showCatagory(__alloyId41);
        var len = models.length;
        var rows = [];
        for (var i = 0; len > i; i++) {
            var __alloyId32 = models[i];
            __alloyId32.__transform = addPath(__alloyId32);
            var __alloyId34 = Ti.UI.createTableViewRow({
                layout: "vertical",
                pid: "undefined" != typeof __alloyId32.__transform["id"] ? __alloyId32.__transform["id"] : __alloyId32.get("id"),
                height: "50%"
            });
            rows.push(__alloyId34);
            var __alloyId36 = Ti.UI.createImageView({
                image: "undefined" != typeof __alloyId32.__transform["imagesWithPath"] ? __alloyId32.__transform["imagesWithPath"] : __alloyId32.get("imagesWithPath"),
                height: "200dp",
                width: "150dp",
                top: "5%"
            });
            __alloyId34.add(__alloyId36);
            var __alloyId38 = Ti.UI.createLabel({
                text: "undefined" != typeof __alloyId32.__transform["name"] ? __alloyId32.__transform["name"] : __alloyId32.get("name")
            });
            __alloyId34.add(__alloyId38);
            var __alloyId40 = Ti.UI.createLabel({
                text: "undefined" != typeof __alloyId32.__transform["price"] ? __alloyId32.__transform["price"] : __alloyId32.get("price")
            });
            __alloyId34.add(__alloyId40);
        }
        $.__views.__alloyId31.setData(rows);
    }
    function showItem(e) {
        var details = Alloy.createController("details", {
            pid: e.row.pid
        });
        Alloy.Globals.tabGroup.activeTab.open(details.getView());
    }
    function showCatagory(collection) {
        return collection.where({
            category: "dresses"
        });
    }
    function addPath(model) {
        transform = model.toJSON();
        transform.imagesWithPath = "/" + transform.images;
        return transform;
    }
    require("alloy/controllers/BaseController").apply(this, Array.prototype.slice.call(arguments));
    this.__controllerPath = "dresses";
    this.args = arguments[0] || {};
    if (arguments[0]) {
        {
            __processArg(arguments[0], "__parentSymbol");
        }
        {
            __processArg(arguments[0], "$model");
        }
        {
            __processArg(arguments[0], "__itemTemplate");
        }
    }
    var $ = this;
    var exports = {};
    var __defers = {};
    Alloy.Collections.instance("items");
    $.__views.win = Ti.UI.createWindow({
        id: "win",
        backgroundColor: "white",
        exitOnClose: "true",
        title: "Dresses"
    });
    $.__views.win && $.addTopLevelView($.__views.win);
    $.__views.__alloyId31 = Ti.UI.createTableView({
        id: "__alloyId31"
    });
    $.__views.win.add($.__views.__alloyId31);
    var __alloyId41 = Alloy.Collections["items"] || items;
    __alloyId41.on("fetch destroy change add remove reset", __alloyId42);
    showItem ? $.addListener($.__views.__alloyId31, "click", showItem) : __defers["$.__views.__alloyId31!click!showItem"] = true;
    exports.destroy = function() {
        __alloyId41.off("fetch destroy change add remove reset", __alloyId42);
    };
    _.extend($, $.__views);
    Alloy.Collections.items.fetch();
    __defers["$.__views.__alloyId31!click!showItem"] && $.addListener($.__views.__alloyId31, "click", showItem);
    _.extend($, exports);
}

var Alloy = require("alloy"), Backbone = Alloy.Backbone, _ = Alloy._;

module.exports = Controller;