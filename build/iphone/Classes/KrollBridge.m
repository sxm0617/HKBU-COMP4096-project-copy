...
NSString * ShoppingApp_V8$ModuleRequireFormat = @"(function(exports){"
...
@implementation ShoppingApp_V8Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_shoppingapp_v8);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ShoppingApp_V8 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ShoppingApp_V8" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_shoppingapp_v8 = [[ShoppingApp_V8Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_shoppingapp_v8];
	
	NSString *_shoppingapp_v8NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _shoppingapp_v8NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_shoppingapp_v8 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ShoppingApp_V8$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.shoppingapp_v8.kroll" 
