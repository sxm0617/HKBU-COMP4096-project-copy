...
//Rare exceptions to indicate a bug in the _shoppingapp_v8 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_shoppingapp_v8 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_shoppingapp_v8]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.shoppingapp_v8.module"
