...
//Rare exceptions to indicate a bug in the _testing code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_testing = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_testing]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.testing.module"
